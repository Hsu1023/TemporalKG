...
class base_HPO():
    def __init__(self, kgeModelName, obj_function, HP_info, dataset_name=None, acq='maxMean', meta_feature=None, msg=None, model_type='single',args=None):
        self.args = args
        ...
        self.pretrain_config_dict = []
        self.trained_config_dict  = []
        self.result_record        = []
        ...
        for cfg in candidate_config:
            for dim in self.HP_info['dim'][1][-2:]:
                for batch_size in self.HP_info['batch_size'][1][-2:]:
                    ...
                    if tmp_cfg not in generated_finetune_config and tmp_cfg not in self.trained_config_dict:
        ...
        if finetuneOtherHP:
            for cfg in candidate_config:
                for i in range(int(sample_num)):
                    ...
                    if tmp_cfg not in generated_finetune_config:
                            generated_finetune_config.append(tmp_cfg)
    ...
    def pretrain_with_meta_feature(self, config_list, mrr_list, dataset_names, meta_feature, topNumToStore=0):
        ...
        if topNumToStore > 0:
            ...
            for idx in topNumidx:
                ...
                self.pretrain_config_dict.append(config_list[idx])

    def BORE_convert_batch_labels(self, Y, valid_index=None):
        ''' Y -> Z for a batch of datasets'''
        
        # if Y.shape[0] != len(dataset_names):
        dataset_names = np.array([self.dataset_name for _ in range(Y.shape[0])])
    ...
    def runTrials(self, maxTrials, sample_num, meta_feature=None):
        ...
        for trial in range(int(maxTrials)):
            logging.info('trials:{}/{}'.format(trial+1, maxTrials))
            ...
            if type(meta_feature) is np.ndarray:
                ...
                candidate_config_X = np.concatenate((candidate_config_X, feature_matrix), axis=1)
            # if rf is trained
            if self.trained_flag and not self.args.randomSearch:
                mu, std = self.predict_with_std(candidate_config_X)
                cfg_index, cfg_pred_y, _ = self.acq_function(mu=mu, std=std)
            else:
                cfg_index  = np.random.randint(len(candidate_config))
                cfg_pred_y = -1
            ...
            self.result_record.append(trial_result['test_mrr'])
            ...
            self.trained_config_dict.append(next_config)
        ...
        return {'X':self.observed_x, 'Y':self.observed_y, 'result_record':self.result_record}

class RF_HPO(base_HPO):
    def __init__(self, kgeModelName, obj_function, HP_info, dataset_name=None, acq='maxMean', meta_feature=None, msg=None,  model_type='single', args=None, current_stage_data=None):        
        base_HPO.__init__(self, kgeModelName, obj_function, HP_info, dataset_name, acq, meta_feature, msg, model_type, args)
        ...
        if current_stage_data is not None:
            self.observed_x = current_stage_data['X']
            self.observed_y = current_stage_data['Y']
            X, Y = np.array(self.observed_x), np.array(self.observed_y)
            valid_index = np.where(Y > 0)
            self.fit(X, Y, valid_index=valid_index)
